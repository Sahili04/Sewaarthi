    ...
    <>
      <div className="greeting s1">
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div>
            <h2>🏃 {t('habits', 'Habit Tracker')}</h2>
            <p>{t('habitsSub', 'Track your daily activities and stay active')}</p>
          </div>
      ...
      <div className="card s2">
        <div className="card-title">⚡ {t('quickAddActivity', 'Quick Add Activity')}</div>
        ...
        <div style={{ display:'flex', gap:8 }}>
          <input className="form-input" value={newName} onChange={e => setNewName(e.target.value)}
            placeholder={t('customActivity', 'Custom activity name...')} onKeyDown={e => e.key==='Enter' && addActivity(newName)}
            style={{ flex:1 }} />
          <button className="btn btn-primary" onClick={() => addActivity(newName)}>{t('add','Add')}</button>
        </div>
      </div>
      ...
      <div className="card s3">
        <div className="section-header">
          <div className="section-title">📋 {t('todayActivities', "Today's Activities")}</div>
          <span style={{ fontSize:12, color:'var(--text3)' }}>{activities.filter(a=>a.done).length}/{activities.length} {t('done','done')}</span>
        </div>

        {activities.length === 0 ? (
          <div className="empty">
            <div className="e-icon">🏃</div>
            <h3>{t('noActivities','No activities yet')}</h3>
            <p>{t('noActivitiesSub','Add an activity to start tracking')}</p>
          </div>
          ...
          <div key={act.id} style={{ display:'flex', alignItems:'center', gap:14, padding:'14px', background: act.done ? 'rgba(0,196,140,0.04)' : 'rgba(255,255,255,0.68)', border:`1px solid ${act.done ? 'rgba(0,196,140,0.2)' : 'rgba(255,255,255,0.9)'}`, borderRadius:16, marginBottom:10, opacity: act.done ? 0.8:1, transition:'all 0.24s', animation:`medIn 0.38s ${idx*0.06}s both` }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ fontSize:11, color:'var(--text3)', marginTop:2 }}>
                ...
                {act.done && <span style={{ marginLeft:8, color:'var(--success)', fontWeight:700 }}>✅ {t('completed','Completed')}</span>}
              </div>
            </div>
            {!act.done && (
